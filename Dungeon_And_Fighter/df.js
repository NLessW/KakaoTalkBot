...
const apiKey = "HoEacyHR7eDWpqpmxzJ2Mf1z7ZXxKcfV";
const serverList = {
"카인":"cain",
"디레지에":"diregie",
"시로코":"siroco",
...
if (msg.startsWith("!중재자 ")) {
  ...
  if (nowScore > maxScore) {
    replier.reply("포인트는 100만을 넘길 수 없습니다.");
      ...
      if (remainScore <= 500) remainRound += 1; // Adjusted condition for ternary operator
...
// Calculating fatigue and mistCore based on day of the week
  if (day === 0 || day === 6) {
    fatigue = 18 * 12; // Sunday or Saturday
    mistCore = 18 * 72;
  } else {
      fatigue = 16 * 12; // Weekdays
      mistCore = 16 * 72;
  }
...
if (msg.startsWith("!서고 ")) {
  var nowScore = parseInt(msg.substr(4).split(" ")[0]); // Adjusted substring start index and added parseInt to convert string to integer
  var maxScore = 1000000;
  if (nowScore > maxScore) {
    replier.reply("포인트는 100만을 넘길 수 없습니다.");
  } else if (nowScore == 0 || isNaN(nowScore)) { // Added isNaN check for invalid input
...
function saveData(adventureName, dataToAdd) {
    ...
    // 파일에서 읽은 데이터가 유효한 JSON 형식인지 확인합니다.
    var existingDataObj;
    try {
    ...
    // adventureName이 이미 존재하는지 확인하고, 존재한다면 해당 adventureName의 데이터를 업데이트합니다.
    if (existingDataObj.hasOwnProperty(adventureName)) {
        ...
        for (var i = 0; i < existingCharacters.length; i++) {
            if (existingCharacters[i].characterName === dataToAdd.characterName) {
                // 이미 존재하는 캐릭터일 경우 해당 캐릭터의 정보를 업데이트합니다.
        ...
        if (!isCharacterExist) {
            // 이미 존재하는 캐릭터가 아닐 경우 데이터를 추가합니다.
            existingDataObj[adventureName].push(dataToAdd);
        }
    } else {
        // adventureName이 존재하지 않는다면 새로운 배열을 생성하여 데이터를 추가합니다.
...
if (msg.startsWith("!모험단 ")) {
    ...
    for (var i = 0; i < adventureInfo.length; i++) {
        ...
        if ("buffScore" in characterInfo) {
            ...
            // buffScore가 없는 경우 rankDamage 출력
...
if (msg.startsWith("!모험단삭제 ")) {
    var characterNameToDelete = msg.substr(7); // 사용자로부터 입력 받은 캐릭터 이름
